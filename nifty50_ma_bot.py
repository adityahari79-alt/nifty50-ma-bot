# nifty50_ma_bot.py
# Streamlit + Dhan API Moving Average Bot for Nifty 50 Deep ITM Call Options
...
# ========== Sidebar: API & Config Input ==========
st.sidebar.header("🔑 Dhan API Configuration")

client_id = st.sidebar.text_input("Dhan Client ID", type="password")
access_token = st.sidebar.text_input("Access Token", type="password")
nifty_security_id = st.sidebar.text_input("Nifty 50 Security ID (Spot)", help="e.g., '1001'")
...
def fetch_5min_candles(dhan: DhanClient, sec_id: str):
    """ Fetch latest intraday data (5 min) from Dhan """
    data = dhan.get_instrument_candle_data(
        security_id=sec_id, exchange_segment="NSE", instrument_type="INDEX", 
        interval="5MIN"
    )
...
def find_deep_itm_ce(dhan: DhanClient, underlying_id: str, expiry: str, target_strike: float):
    """Find the option security_id for deep ITM CE"""
    ...
    for opt in chain:
        if (opt['strike_price'] == target_strike) and (opt['option_type'] == "CE"):
    ...
    if not (client_id and access_token and nifty_security_id and expiry_date):
        ...
        while True:
            ...
            if last_candle['ma10'] >= last_candle['ma21']:
                ...
                while True:
                    try:
                        ltp_data = dhan.get_security_quote("NSE_FNO", option_id)
